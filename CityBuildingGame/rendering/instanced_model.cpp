...
InstancedMesh InstancedModel::processMesh(aiMesh *mesh, const aiScene *scene)
{
	...
	// 4. height maps
	std::vector<imTexture> heightMaps = loadMaterialTextures(material, aiTextureType_AMBIENT, "texture_height");
...
unsigned int InstancedModel::TextureFromFile(const char *path, const string &directory, bool gamma)
{
	...
	if (data)
	{
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);//GL_LINEAR);// GL_NEAREST_MIPMAP_LINEAR);// GL_LINEAR_MIPMAP_LINEAR);
